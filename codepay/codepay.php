...
if($_GET['id']) {
    $m = new M();
    $sql = "select * from `dt_recharge` where order_no=?";
    $order_info = $m->getOne($sql, array('s', $_GET['id']), false);
    if ($order_info != false) {

        $pay_id = $order_info['order_no']; //网站唯一标识 需要充值的用户名，用户ID或者自行创建订单 建议传递用户的ID
        $price = (float)$order_info["price"]; //提交的价格
    }
}
...
function getCodepayHtml($parameter,$key,$gateway='http://api2.xiuxiu888.com/creat_order/?',$https_gateway='https://api.xiuxiu888.com/creat_order/?'){
    $codepay_json='';
