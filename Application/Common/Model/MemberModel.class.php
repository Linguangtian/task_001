...
class MemberModel extends BaseModel
{
     ...
     * @return bool
     */
    public function incPoint($member_id, $point, $type=0, $remark = '', $no='')
    {
        if( !($point>0) ) {
            return false;
        }
        $res = M('member')->where(array('id' => $member_id))->setInc('point', $point);
        $res = M('member')->where(array('id' => $member_id))->setInc('jia_point', $point);
        if ($res) {
     ...
     * @return bool
     */
    public function decPoint($member_id, $point, $type=0, $remark = '', $no='')
    {
        if( !($point>0) ) {
            return false;
        }
        $res = M('member')->where(array('id' => $member_id))->setDec('point', $point);
        $res = M('member')->where(array('id' => $member_id))->setInc('jian_point', $point);
        if ($res) {
     ...
     * @param $member_id
     * @param $price
     * @param int $type 1注册 2签到 3当日首次任务 4未在规定时间内完成 5未按要求完成
     * @param string $remark
     */
    private function point_log($member_id, $price, $type=0, $remark = '', $no='')
    {
        $data['member_id'] = $member_id;
        $data['point'] = $price;
        $data['remark'] = $remark;
