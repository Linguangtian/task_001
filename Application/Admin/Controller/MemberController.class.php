...
class MemberController extends AdminBaseController{
    ...
    public function index() {
        ...
        foreach($list as &$_list) {
            ...
            $_list['p1_name'] = M('member')->where(['id' => $_list['p1']])->getField('username');
        }
...
//        unset($member_level[3]);
    ...
    public function recharge()
    {
       ...
       if($this->limit_menber_reg($level)){
        ...
        /*是否补差价*/
        $user_level =   M('member')->where(['id' => $member_id])->getField('level');
        if($user_level>0){
            $diff_price =$price - $member_level[$user_level]['price'];
            $price =$diff_price>0?$diff_price:0;
        }
    ...
    /**
     * xiao5    2019年7月9日11:17:36   信用分充值
     */
    public function point_chongzhi()
    {
        if( IS_POST ) {
            $member_id = intval(I('post.member_id'));
            $price = floatval(I('post.price'));
            ...
            if( $price>0 ) {
                $mark = '管理员后台充值，管理员ID：'.session('user.id');
                $res = $model_member->incPoint($member_id,$price,6,$mark);
            } else {
                $member_point = M('member')->where(['id' => $member_id])->getField('point');
                if ($member_point < abs($price)) {
                    $this->error('用户信用分不足');
                }
                $mark = '管理员后台充值，管理员ID：'.session('user.id');
                $res = $model_member->decPoint($member_id,abs($price),7,$mark);
            }
    ...
    public function team_show() {
        ...
        //一级
        $list1 = $model->alias('u')->field('u.id,u.nickname,u.username,u.head_img,u.create_time,l.name  as level_name')
               ->join(C('DB_PREFIX').'level as l on u.level = l.level','left')
			->where(array('u.p1'=>$id))
            ->order('u.id desc')
            ->select();
        ...
        //二级
        $list2 = $model->alias('u')->field('u.id,u.nickname,u.username,u.head_img,u.create_time,l.name  as level_name')
		  ->join(C('DB_PREFIX').'level as l on u.level = l.level','left')
            ->where(array('u.p2'=>$id))
            ->order('u.id desc')
            ->select();
        ...
        //三级
        $list3 = $model->alias('u')->field('u.id,u.nickname,u.username,u.head_img,u.create_time,l.name  as level_name')
		  ->join(C('DB_PREFIX').'level as l on u.level = l.level','left')
            ->where(array('u.p3'=>$id))
            ->order('u.id desc')
            ->select();
