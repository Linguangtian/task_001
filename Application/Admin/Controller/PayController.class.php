...
class PayController extends AdminBaseController{
    ...
    public function index() {
        ...
        if( I('get.platform_no')!='' ) $map['a.order_no'] = I('get.platform_no');
		if( I('get.member_name')!='' ){
            $map['c.username']=['like',I('get.member_name')];
        }
        //搜索时间
        ...
        $model = M('pay');
		$count = $model->alias('a')->join(C('DB_PREFIX').'member as c on a.member_id = c.id','left')->where($map)->count();
        ...
        //总金额
        $total_price = $model->alias('a')->join(C('DB_PREFIX').'member as c on a.member_id = c.id','left')->where($map)->sum('a.price');
    ...
    public function point_log() {
        ...
        $model = M('member_point_log')->alias('a');
        $count = $model->where($map)->count();
        $page = sp_get_page($count, 20);//分页
        $firstRow = $page->firstRow;
        $listRows = $page->listRows;

        $list = M('member_point_log')->alias('a')->join(C('DB_PREFIX').'member as c on a.member_id = c.id','left')
            ->where($map)
            ->field('a.*,c.username,c.nickname,c.phone,c.bank_name,c.bank_user,c.bank_number')
            ->order('a.id desc')->limit("$firstRow , $listRows")
            ->select();
    ...
    public function tixian() {
        ...
        $list = M('member_tixian')->alias('a')->join(C('DB_PREFIX').'member as c on a.member_id = c.id','left')
            ->where($map)
            ->field('a.*,c.nickname,c.phone,c.bank_name,c.bank_user,c.bank_number,c.username,c.phone')
            ->order('a.id desc')->limit("$firstRow , $listRows")
            ->select();
        ...
        foreach( $list as &$_list ) {
            ...
            $_list['charge'] =  $_list['price'] -  $_list['actual_price'];
            $_list['nickname'] =  $_list['nickname']?$_list['nickname']: $_list['username'];
