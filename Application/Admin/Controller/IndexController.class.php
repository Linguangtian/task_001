...
class IndexController extends AdminBaseController{
	...
	public function tongji(){
        ...
        $this->assign('total_sh',$total_sh);
		 ...
		 if(IS_POST){
            $start_date=I('start_date')?strtotime(I('start_date')):0;
            $end_date=I('end_date')?strtotime(I('end_date')):0;

            $map=[];
            $task_apply_map=['status'=>2];
            $tixian_map=['status'=>1];
            $member_price_map=[];
            if($start_date){
                $map['create_time']=['gt',$start_date];
                $task_apply_map['update_time']=['gt',$start_date];
                $tixian_map['update_time']=['gt',$start_date];
            }


            if($end_date){
                $map['_string'] = "( create_time <= {$end_date}  )";
                $task_apply_map['_string'] = "( update_time <= {$end_date}  )";
                $tixian_map['_string'] = "( update_time <= {$end_date}  )";

            }

            $total_num['member'] = M('member')->where($map)->count();
            $total_num['task'] = M('task')->where($map)->count();
            $total_num['task_apply'] = M('task_apply')->where($task_apply_map)->count();
            $total_num['pay'] = M('pay')->where($map)->sum('price');
            $total_num['tixian'] = M('member_tixian')->where($map)->sum('price');


            $total_num['pay']=$total_num['pay']?$total_num['pay']:0;
            $total_num['tixian']=$total_num['tixian']?$total_num['tixian']:0;

            $data=['error'=>0,'total_num'=>$total_num];
