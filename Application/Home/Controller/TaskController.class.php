...
Eof;
    ...
    public function handle() {
        ...
        if( IS_POST ) {
            ...
            $data = I('post.');
            if(!$data['price']||!$data['max_num']){

                $this->error ('参数错误!');
            }

            $total_cost = floatval($data['price']) *  floatval($data['max_num']);

            if($total_cost > $member['price']){
          ...
          if( !($id > 0) ) {
                ...
                if ( $model->add ($data) ) {
                    ...
                    ////扣除金额
                     ...
                     $model->decPrice($this->member_id, $total_cost, 101, '发布任务');
                    header('location:'.U('Home/Member/index',array('msg'=>'新增成功,等待审核!')));
                 //    $this->success ('新增成功,等待审核!', U('index'));
                } else {
                    $this->error ('新增失败!');
                }
            } else {
